...
public:
    int findDuplicate(vector<int>& nums) {
        // using hare and tortoise (slow and fast technique)
        int slow = 0; // one step at a time
        int fast = 0; // two step at a time
...
//9 differnet approaches to solve this problem of duplicatre number
...
It's easy to use 2 loops to do it, but the time complexity is O(n 
2
...
Bit 1	1	0	1	1	1	3	2
Bit 2	0	0	1	0	0	1	1
From the table, we found that only the 1th bit x>y, so after bitwise restoration target=(010) 
2
​
 =(2) 
10
...
If target appears twice in the test case array, the rest of the numbers appear once, and the ith bit of target is 1, then the nums array x, is exactly one greater than y. If bit i of \textit{target} is 0, then both are equal.
